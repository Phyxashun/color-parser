...
import Tokenizer, { DiagnosticError, LexerError, formatDiagnostic } from '../src/NewLexer.ts';
...
describe('DiagnosticError', () => {
    it('should create error with all parameters', () => {
        const error = new DiagnosticError(
            ...
            { index: 5, line: 1, column: 6 },
            'Test hint'
        );

        expect(error.message).toBe('Test error');
        expect(error.source).toBe('source code');
        expect(error.start).toEqual({ index: 0, line: 1, column: 1 });
        expect(error.end).toEqual({ index: 5, line: 1, column: 6 });
        expect(error.hint).toBe('Test hint');
        expect(error.name).toBe('DiagnosticError');
    });

    it('should create error without optional parameters', () => {
        const error = new DiagnosticError('Test error', 'source code');
...
describe('LexerError', () => {
    it('should create lexer error', () => {
        const error = new LexerError('Lexer error', 'source code', 42);

        expect(error.message).toBe('Lexer error');
        expect(error.source).toBe('source code');
        expect(error.index).toBe(42);
        expect(error.name).toBe('LexerError');
    });

    it('should extend SyntaxError', () => {
        const error = new LexerError('Lexer error', 'source', 0);
...
describe('formatDiagnostic', () => {
    it('should format diagnostic with all information', () => {
        const error = new DiagnosticError(
            'Unexpected token',
            'const x = @;',
            { index: 10, line: 1, column: 11 },
            { index: 11, line: 1, column: 12 },
            'Remove the @ symbol'
        );
        ...
        expect(formatted).toContain('line 1, column 11');
        expect(formatted).toContain('const x = @;');
        expect(formatted).toContain('Hint: Remove the @ symbol');
    });

    it('should format diagnostic without hint', () => {
        const error = new DiagnosticError(
            'Error message',
            'code',
            { index: 0, line: 1, column: 1 },
    ...
    it('should handle missing position info', () => {
        const error = new DiagnosticError('Error', 'source');

        const formatted = formatDiagnostic(error);

        expect(formatted).toContain('Error');
        expect(formatted).toContain('line 0, column 0');
    });

    it('should format caret correctly for single character', () => {
        const error = new DiagnosticError(
            'Error',
            'abc',
            { index: 1, line: 1, column: 2 },
    ...
    it('should format caret correctly for multi-character span', () => {
        const error = new DiagnosticError(
            'Error',
            'hello world',
            { index: 0, line: 1, column: 1 },
    ...
    it('should handle multi-line source', () => {
        const error = new DiagnosticError(
            'Error',
            'line1\nline2\nline3',
            { index: 6, line: 2, column: 1 },
            { index: 11, line: 2, column: 6 }
        );
    ...
    it('should ensure minimum caret width of 1', () => {
        const error = new DiagnosticError(
            'Error',
            'x',
            { index: 0, line: 1, column: 1 },
